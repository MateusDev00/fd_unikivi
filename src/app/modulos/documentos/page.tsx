  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="relative max-w-md">
        ...
        <input
          type="text" placeholder="Pesquisar documentos..." value={searchTerm}
            ...
            >
              <div>
                <div className="flex items-start gap-3">
                  <div className="p-2 bg-primary-light rounded-lg"><FileText className="h-5 w-5 text-primary" /></div>
                  <div>
                    <h3 className="font-medium text-heading line-clamp-2">{doc.titulo}</h3>
                    <p className="text-xs text-body mt-1">{formatSize(doc.tamanho_bytes)} • {doc.tipo_mime?.split('/')[1]?.toUpperCase()}</p>
                  </div>
                </div>
              </div>
              <div className="flex justify-end gap-2 mt-4">
                <Button variant="ghost" size="sm" onClick={() => setSelected(doc)}><Eye className="h-4 w-4 mr-1" /> Detalhes</Button>
                <Button variant="outline" size="sm" onClick={() => window.open(`${process.env.NEXT_PUBLIC_API_URL}/documentos/${doc.id}/download`, '_blank')}>
        ...
        <Modal isOpen={!!selected} onClose={() => setSelected(null)} title={selected.titulo}>
          <div className="space-y-2 text-sm">
            ...
            <div className="flex gap-2 mt-4">
              <Button onClick={() => window.open(`${process.env.NEXT_PUBLIC_API_URL}/documentos/${selected.id}/download`, '_blank')}>
                <Download className="h-4 w-4 mr-1" /> Download
              </Button>
            </div>
          </div>
