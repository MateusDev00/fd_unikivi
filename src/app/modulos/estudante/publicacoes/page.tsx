...
export default function PublicacoesEstudante() {
    ...
    <div className="space-y-8 animate-fade-in">
                ...
                <div className="flex items-center justify-between mt-4">
                  <span
                    className={`text-xs px-2 py-1 rounded-full ${
                      pub.estado === 'publicado' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-700'
                    }`}
                  >
                    {pub.estado}
                  </span>
      ...
      {selected && (
        <Modal isOpen={!!selected} onClose={() => setSelected(null)} title={selected.titulo}>
          {selected.imagem_capa && (
            <img src={selected.imagem_capa} alt={selected.titulo} className="w-full rounded-lg mb-4" />
          )}
