...
import {
  Scale, GraduationCap, BookOpen, Globe, Users, Award, ChevronRight, FileText, Download, Eye
} from 'lucide-react';
...
export default function SobrePage() {
  const [activeTab, setActiveTab] = useState<'sobre' | 'documentos'>('sobre');
  ...
  return (
    <>
      ...
      <main className="pt-20">
        ...
        <div className="container mx-auto px-4 mt-12">
          <div className="flex justify-center gap-4 border-b border-gray-200 pb-4">
            <button
              ...
              className={`px-8 py-3 rounded-t-xl font-serif text-lg transition-all ${
                activeTab === 'sobre'
                  ? 'bg-primary text-white shadow-md -mb-[1px] relative z-10'
                  : 'text-body hover:text-heading hover:bg-gray-100'
              }`}
            >
              Sobre a FD‑UNIKIVI
            </button>
            ...
            <section className="py-16 bg-white">
              <div className="container mx-auto px-4 max-w-4xl">
                ...
                >
                  ...
                  <p className="text-body leading-relaxed text-lg mb-4 text-justify">
                  ...
                  <p className="text-body leading-relaxed text-lg text-justify">
        ...
        {/* Conteúdo da tab Documentos Legais */}
        {activeTab === 'documentos' && (
          <section className="py-16 bg-gray-50">
            <div className="container mx-auto px-4">
              <h2 className="font-serif text-3xl text-heading mb-8 text-center">Documentos Legais</h2>
              {loadingDocs ? (
                ...
                <p className="text-center text-body">Nenhum documento disponível.</p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {documentos.map((doc) => (
                    <motion.div
                    ...
                    >
                      ...
                      <div className="flex justify-end gap-2 mt-4">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => window.open(`/api/documentos/${doc.id}/download`, '_blank')}
                        >
        ...
        {selectedDoc && (
          <Modal isOpen={!!selectedDoc} onClose={() => setSelectedDoc(null)} title={selectedDoc.titulo}>
            <div className="space-y-2 text-sm">
              <p><strong>Nome original:</strong> {selectedDoc.nome_original}</p>
              <p><strong>Tamanho:</strong> {formatSize(selectedDoc.tamanho_bytes)}</p>
              <p><strong>Tipo:</strong> {selectedDoc.tipo_mime}</p>
              {selectedDoc.descricao && <p><strong>Descrição:</strong> {selectedDoc.descricao}</p>}
              <div className="flex gap-2 mt-4">
                <Button onClick={() => window.open(`/api/documentos/${selectedDoc.id}/download`, '_blank')}>
                  <Download className="h-4 w-4 mr-1" /> Download
                </Button>
              </div>
            </div>
